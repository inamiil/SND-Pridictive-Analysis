...
division_query = """
SELECT  REGION, DIVISION, MONTH, YEAR, NET_SALE_AMOUNT
FROM FINAL_QUERY
"""

data = run_query(division_query)
df = pd.DataFrame(data)

# === Set Colors ===
BISCONNI_COLOR = '#1f77b4'
CANDYLAND_COLOR = '#ff7f0e'
COMBINED_COLOR = '#7851A9'
# === Quick EDA Summary ===
...
for division in heatmap_data['DIVISION'].unique():
    pivot = heatmap_data[heatmap_data['DIVISION'] == division] \
        .pivot(index='YEAR', columns='MONTH', values='NET_SALE_AMOUNT') \
        .fillna(0)

    pivot = pivot.apply(pd.to_numeric, errors='coerce')  # Ensure numbers
